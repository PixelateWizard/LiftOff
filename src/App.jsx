...
import { useLibraryScanPhase } from "./hooks/useLibraryScanPhase";
...
export default function App() {
  ...
  const {
    ...
    customHeroArtRef,
    loadCustomArt,
    fetchGameArt: fetchGameArtRaw,
    clearGameArt,
  } = useCustomArt();
  const [artSetupProgress, setArtSetupProgress] = useState(null);
  const artSetupJobRef = useRef(0);
  ...
  const libraryScanPhase = useLibraryScanPhase(libraryRefreshStatus === "scanning");
  const leftoverArtSetup = useMemo(
    () => countLibraryArtSetup(apps, gameArt, heroStatic, {
      includeUninstalled: showUninstalledGames,
      customArt,
      customHeroArt,
    }),
    [apps, customArt, customHeroArt, gameArt, heroStatic, showUninstalledGames],
  );
  const barSetup = useMemo(
    () => resolveBarSetup(
      loading,
      libraryRefreshStatus === "scanning",
      libraryScanPhase,
      artSetupProgress && artSetupProgress.done < artSetupProgress.total ? artSetupProgress : leftoverArtSetup,
    ),
    [loading, libraryRefreshStatus, libraryScanPhase, artSetupProgress, leftoverArtSetup]
  );
  const {
  ...
  useEffect(() => {
    if (tab === "Settings") return;
    const libraryVertical = tab === "Games" || tab === "Apps";
    const scrollBehavior = navRepeatingRef.current ? "auto" : "smooth";
    const scrollVertically = (scroller, top, behavior = scrollBehavior, minDelta = 1) => (
      libraryVertical
        ? scrollElementToRetargeted(scroller, top, behavior, minDelta)
        : scrollElementTo(scroller, top, behavior, minDelta)
    );
    ...
    const scrollToTop = (behavior = scrollBehavior) => {
      ...
      scrollVertically(scroller, 0, behavior);
      if (tab === "Home") scrollElementTo(outerRef.current, 0, behavior);
    };
    const scrollFocusedCardIntoView = () => {
      ...
      scrollVertically(scroller, newTop, scrollBehavior, libraryVertical ? 8 : 1);
    };
    ...
    if (focusSection === "hero") {
      ...
      if (visiblePinnedAboveGrid && focusedCardRef.current) {
        ...
        const scroller = tab === "Home" ? homeScrollRef.current : tabScrollRef.current;
        scrollVertically(scroller, 0, scrollBehavior);
      }
    } else if (focusedCardRef.current) {
      scrollFocusedCardIntoView();
    }
  ...
  return (
    ...
    <div data-theme={resolvedTheme} data-motion={motionProfile} data-ui-motion={settings.ui_motion === false ? "off" : "on"} data-effects={settings.stars_enabled === false ? "static" : "animated"} data-focus={windowFocused && !fseSessionActive ? "active" : "blurred"} className={launchingApp ? "app-launch-paused" : undefined} style={{ ...materialTokens, "--accent-pulse": `${accent.glow}0.22)`, "--header-height": `${headerHeightVal}px`, "--bottom-bar-height": `${bottomBarHeightVal}px`, position: "fixed", top: 0, left: 0, width: `${100 / (settings.ui_scale ?? 1)}vw`, height: `${100 / (settings.ui_scale ?? 1)}vh`, transform: `scale(${settings.ui_scale ?? 1})`, transformOrigin: "top left", overflowY: (semiHomeShellActive || immersiveHomeShellActive) ? "hidden" : "auto", overflowX: "hidden", animation: "appFadeIn 0.5s ease forwards", zIndex: 1, fontFamily: "'Segoe UI', sans-serif" }} ref={outerRef}>
      ...
      {detailsApp && detailsApp.app_type === "game" && (
        <GameDetailsModal
          app={detailsApp}
      ...
      {detailsApp && detailsApp.app_type !== "game" && (
        <AppDetailsModal
          app={detailsApp}
          coverArt={customArt[detailsApp.id]}
          sizeBytes={installSize[detailsApp.id] === null ? undefined : installSize[detailsApp.id]}
          running={isRunning(detailsApp.id)}
          hapticEnabled={settings.haptic_feedback ?? true}
          isPinned={pins.includes(detailsApp.id)}
          isHidden={hidden.includes(detailsApp.id)}
          onOpen={() => { triggerLaunch(detailsApp, recentRef.current); closeDetailsModal(false); }}
          onClose={closeDetailsModal}
          onTogglePin={() => togglePin(detailsApp)}
          onToggleHidden={() => toggleHidden(detailsApp.id)}
          onChangeArt={() => {
          ...
          onMoveToGames={() => {
            applyCategoryOverride(detailsApp.id, "game", detailsApp.source === "uwp" ? "xbox" : null);
            closeDetailsModal(false);
          }}
          onResetCategory={categoryOverrides[detailsApp.id] ? () => { applyCategoryOverride(detailsApp.id, null, null); closeDetailsModal(false); } : undefined}
          ...
          onCloseApp={isRunning(detailsApp.id) ? () => {
            const app = detailsApp;
            closeDetailsModal(false);
            setCloseRequest({ app, force: false });
          } : undefined}
