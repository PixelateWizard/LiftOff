...
import { collectDownloads, pickBarActivity, type BarActivity, type BarDownload, type BarRunning, type BarSetup } from "../utils/smartBar";
...
export function useBarActivity({
  ...
  const activity = useMemo(
    () => pickBarActivity(
      downloads,
      running.filter((entry) => entry.app.app_type === "game"),
      { available: updateAvailable, version: updateVersion },
      setup,
    ),
    [downloads, running, updateAvailable, updateVersion, setup]
  );
