...
import { installPhaseLabelKey, runningMinutes, setupScanLabelKey, type BarActivity, type BarHint } from "../../utils/smartBar";
...
export const SmartPill = memo(function SmartPill({
  ...
  const renderActivity = () => {
    ...
    } else if (activity.kind === "setup") {
      const setup = activity.setup;
      if (setup.kind === "scan") {
        title = String(t("smartBar.activity.setupLibrary"));
        subtitle = String(t(setupScanLabelKey(setup.scanPhase)));
      } else {
        title = setup.lastName || String(t("smartBar.activity.setupArt"));
        subtitle = String(t("smartBar.activity.setupArtProgress", { done: setup.done, total: setup.total }));
      }
      icon = <span className="lo-loading-spinner" style={{ width: 22, height: 22, borderWidth: 2.5, color: accent.primary }} />;
