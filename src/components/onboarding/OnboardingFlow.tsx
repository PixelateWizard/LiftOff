...
interface OnboardingFlowProps {
  ...
  commitSettings: (updates: Partial<Settings>) => Promise<void> | void;
...
export function OnboardingFlow({
  ...
  const closeFlow = useCallback(() => {
    ...
    const finalSave = commitSettings({ ...draftRef.current, onboarding_complete: true });
    draftRef.current = {};
    const finish = () => {
      onFinish({
    ...
    void Promise.resolve(finalSave).then(finish, finish);
  ...
  const applyFocusValue = useCallback((index: number) => {
    ...
    if (currentStep === "theme") {
      ...
      const scene = LOFI_SCENE_OPTIONS[index];
      if (scene) setDraft({ lofi_scene: scene } as Partial<Settings>);
  ...
  const moveCursor = useCallback((direction: Direction) => {
    ...
    if (currentStep === "lofi_scene" && current === LOFI_MUSIC_INDEX && direction === "up") {
      next = Math.max(0, LOFI_SCENE_OPTIONS.indexOf(resolveLofiScene(settingsRef.current.lofi_scene)));
    }
    if (next === current) return;
    cursorRef.current = next;
    setCursor(next);
    playSound();
    if (shouldPreviewFocusedValue(currentStep, next)) applyFocusValue(next);
  ...
  const activate = useCallback(() => {
    ...
    if (currentStep === "lofi_scene") {
      if (cursorRef.current < LOFI_SCENE_OPTIONS.length) {
        rumble("confirm", settingsRef.current.haptic_feedback !== false);
        applyFocusValue(cursorRef.current);
        playSoundAlt();
        return;
      }
      const musicOn = settingsRef.current.lofi_music_enabled !== false;
      setDraft({ lofi_music_enabled: !musicOn } as Partial<Settings>);
      playSoundAlt();
      return;
    }
    if (currentStep === "home") {
  ...
  const renderBody = () => {
    ...
    if (step === "lofi_scene") {
      const musicOn = settings.lofi_music_enabled !== false;
      return (
        <div className="lo-onb-grid" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <LofiSceneGrid
            selected={resolveLofiScene(settings.lofi_scene)}
            focusedIndex={cursor}
            onFocus={(index) => focusCard(index)}
            onSelect={(key) => {
              const index = LOFI_SCENE_OPTIONS.indexOf(key);
              focusCard(index);
              applyFocusValue(index);
              playSoundAlt();
            }}
            onActivate={activate}
          ...
          />
          <div
            className="lo-onb-card"
            onClick={() => { focusCard(LOFI_MUSIC_INDEX); activate(); }}
            onMouseMove={() => focusCard(LOFI_MUSIC_INDEX)}
            style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, padding: "14px 18px", cursor: "pointer", borderRadius: surfaceStyle === "win9x" ? 0 : 12, background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)", ...tileOutline(cursor === LOFI_MUSIC_INDEX, false) }}
          >
            <span style={{ fontSize: 14, fontWeight: 500, color: theme.text }}>{t("settings.lofiMusic")}</span>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: musicOn ? accent.primary : theme.textFaint }}>{musicOn ? t("onboarding.on") : t("onboarding.off")}</span>
              <ToggleKnob value={musicOn} />
            </div>
          </div>
        </div>
      );
            ...
            {HOME_MODE_OPTIONS.map((value, index) => {
              ...
              return (
                <div key={value} className="lo-onb-card" data-home-mode={value}
                  ...
                  <div style={{ width: "100%", aspectRatio: "16 / 9", overflow: "hidden", background: isDark ? "rgba(0,0,0,0.45)" : "rgba(0,0,0,0.08)" }}>
                    <img
                      src={HOME_MODE_PREVIEWS[value]}
                      alt=""
                      draggable={false}
    ...
    return (
      <div style={{ padding: "16px 4px", fontSize: 14, lineHeight: 1.7, color: theme.text }}>
        ...
        <p style={{ margin: "0 0 12px", color: theme.textDim, fontSize: 13 }}>{t("onboarding.done.art")}</p>
  ...
  return (
    <div data-theme={resolvedTheme} data-ui-motion={settings.ui_motion === false ? "off" : "on"} className="lo-onb-overlay"
      ...
      <div className="lo-onb-scrim" style={modalScrimStyle} />
