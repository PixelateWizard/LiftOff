...
import { ACCOUNT_ROWS, ESSENTIAL_ROWS, HOME_MODE_OPTIONS, HOME_ROWS, LANGUAGE_OPTIONS, LOFI_MUSIC_INDEX, PROGRESS_STEPS, SOURCE_ROWS, STEP_ORDER, TAB_ICON_OPTIONS, VISUAL_ROWS, homeModeCardIndex, isSingleSelect, shouldPreviewFocusedValue, shouldSkipOnboardingStep, stepCount, tabIconModeCardIndex, visualTabbarTriggersOn, visualTopBumpersOn, visibleProgressSteps } from "./steps";
...
describe("onboarding row translations", () => {
  for (const [locale, resources] of Object.entries({ en, fr, es })) {
    it(`resolves every Home, source, account, and essential label in ${locale}`, () => {
      ...
      const artValue = ["onboarding", "done", "art"].reduce<any>((node, part) => node?.[part], resources);
      expect(typeof artValue, "onboarding.done.art").toBe("string");
      expect(artValue.length, "onboarding.done.art").toBeGreaterThan(0);
    });
  }
  ...
  it("offers Lo-fi scenes after theme and skips that step for other themes", () => {
    ...
    expect(stepCount("lofi_scene")).toBe(LOFI_SCENE_OPTIONS.length + 1);
    expect(LOFI_MUSIC_INDEX).toBe(LOFI_SCENE_OPTIONS.length);
    expect(isSingleSelect("lofi_scene")).toBe(false);
    expect(shouldPreviewFocusedValue("lofi_scene", 4)).toBe(false);
    expect(shouldPreviewFocusedValue("theme", 4)).toBe(true);
