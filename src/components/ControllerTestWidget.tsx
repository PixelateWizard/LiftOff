...
let _cachedGpSnap: any = null;

export function ControllerTestWidget({ accent, theme, isDark, glass }: any) {
    ...
    const poll = () => {
      ...
      const next = best
        ? {
            ...
            buttons: Array.from(best.buttons).map((b: any) => b.pressed),
            axes: Array.from(best.axes).map((v: any) =>
              typeof v === "number" && isFinite(v) ? v : 0
            ),
            allDevices: all.map((gp: any, i: number) => ({
              index: i,
              name: gp.id,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {gpSnap.buttons.map((pressed: boolean, i: number) => (
          <div
            key={i}
            style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "5px 14px" }}>
        {gpSnap.axes.map((val: number, i: number) => (
