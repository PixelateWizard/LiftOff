...
- Home hero media has to be rasterized outside the UI-scale transform. `ui_scale` above 1 lays the app out smaller and bitmap-scales the root; counter-scaling inside that root still resamples the image. Full-bleed Home heroes (and square-cornered ones) are portaled to the real viewport, and the theme background gets a matching hole so the image shows through. Scrims in the scaled root still paint over it. Cyberpunk's Legacy card keeps its filled frame inside the root. When UI scale is above 1, the portal is sized in device pixels: a still that can cover the frame uses cover, and a shorter one stays at its own pixels, top-aligned, with the hole shrunk to that box so the theme shows around it. Uploads keep their own shape inside 3840×2160 and are never enlarged. A hero already saved at 1920×620 or 3840×1240 stays that size until the user saves it again. Do not put `translateZ` or another transform on hero media that stays inside the UI-scale root: that pair resamples the picture and softens it, including at scale 1.
