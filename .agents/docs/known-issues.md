...
### Home and appearance-picker controller focus

Unit coverage establishes row-bounded horizontal and column-aware vertical grid movement, while code inspection confirms that Home, theme, and surface cards no longer react merely because programmatic scrolling moved them beneath a stationary pointer. Physical Ally/WebView2 validation remains required after returning from a Steam game: traverse beyond both visible edges of the Normal Home row with the pointer parked over the shelf and confirm the focused title advances one item at a time without jumping backward. In both appearance pickers, test D-pad and left-stick taps plus held repeat across every row, including the surface grid's incomplete final row; focus must follow visible geometry, remain on-screen, and select the highlighted item with A.

### First-run onboarding runtime validation

Follow-up regression coverage exercises pad-driven footer navigation at 1280x800, source/essential label resolution, fresh Home with and without detected games, and Data joystick scrolling with a long mocked drive list. Physical acceptance must include Down from the final body row to Next, Left to Back, Up back into the body, Done/Back on the final step, and both joystick directions in Data without storage-row reloads. These mocks do not establish physical controller feel.

Builds and automated tests cover the persisted fresh-install/upgrade split, frontend wiring, locale parity, and main-navigation gates, but real Ally/WebView2 validation remains required. Delete only a disposable test profile's settings file, then confirm onboarding appears after the splash without flashing for an existing pre-onboarding file or when screen-resolution lookup falls back. Exercise every step with a physical controller, including haptics, held repeat, locked-theme surface skipping, the Lo-fi scene pictures after Lo-fi is selected (and skipping that step for other themes), live background compositing, exactly one source refresh, Settings re-run, and Y skip. For each existing account dialog, verify its real credential round-trip and confirm opening/closing edges never move or reactivate onboarding underneath. Steam QR, Microsoft, and Spotify connect dialogs now render outside the scaled app root so they can stack above setup; mocked Chromium coverage checks that path. Repeat direct cold-boot FSE separately from desktop-to-FSE; mocked pads and browser tests do not close the double-input, live-account, WebView2, or FSE gates.

Settings factory reset must not delete `%LOCALAPPDATA%/LiftOff` while WebView2 still maps art files: that path failed in-session, left settings in place, and broke Home hero images. Confirm on a disposable profile that A/click on Reset LiftOff never bleeds into Settings, the app restarts into first-run onboarding with empty settings/pins/recents/art, WebView storage is cleared, and Steam, Microsoft, and Spotify accounts are disconnected. Splash must leave “Reading Start Menu and Desktop shortcuts” instead of hanging on a cold icon-cache wipe. If restart itself does not relaunch, closing and reopening the app must still apply the pending wipe.

### Helper bar native controls and controller surfaces

The modal surface audit now preserves shared surface tokens in helper, standard, account, art, file, keyboard, loading, and search shells. Mocked Chromium coverage compares helper, standard Controls, and refresh panel fills/filters/borders/shadows across Glass, Aero, Material, Clear, Obsidian, Cyberpunk/Neon, Win9X, and Onyx. Complete the equivalent WebView2 visual pass over bright game art, including custom/nested dialogs, effects-off and reduced motion; static audit and representative browser screenshots do not prove every native modal combination.

Compilation, unit tests, and mocked-browser smoke coverage establish the command contracts, settings migration, tray composition, and app-poll gating, but Ally/WebView2 validation remains required. Confirm on the ROG Ally that Core Audio reads and changes the active render endpoint without flyout spam; raising volume unmutes; WMI brightness appears, changes the internal panel on AC and battery, and stays hidden on external-monitor-only systems; Smart/Full/Hidden layouts paint correctly at 1280x800 across representative Glass/Material/win9x surfaces; the shortcut/system-control/pinned/Spotify priority order remains legible, sufficiently opaque over bright game art, and stable as focus moves; long pinned rows keep the focused shortcut visible; pinned games and apps open Details without A-button bleed; hidden hover and activity peeks time out correctly; Spotify transport/seek and playlist entry points behave for Premium, Free, disconnected, and no-track states; single-press MENU tray access in every bar mode, A-gated scrubber/system-control adjustment, adjacent-control navigation, D-pad repeat, B close, Controls LB/RB, and tray-to-Power/modal transitions never leak an activation underneath.

### Lo-fi scene video playback

Settings, locale, and `<video>` reload wiring cover `lofi_scene` (`cozy`, `dog`, `desk`, `cat`, `rainy_street`, `pixel_shop`) plus the Settings picture picker, the onboarding scene step when Lo-fi is selected, inlined scene posters, local-file video/mp3 via `%LOCALAPPDATA%/LiftOff/media/lofi/`, and the onboarding `lofi_music_enabled` switch. Ally X WebView2 validation remains required after a factory reset: first-run Lo-fi posters must paint, A must select a scene without skipping to the next step, the selected scene video and looping mp3 must start, turning music off on that step must persist, effects-off pauses the clip, a non-`cozy` choice survives restart, and the scene step appears only after Lo-fi is picked.

### Smart bar collapse and session timing

Smart bar collapse/expand width transition over backdrop-filter on Ally X WebView2 needs a hardware check for blur settle/flicker. Running session time is measured from when LiftOff first saw the process, not true session start.
