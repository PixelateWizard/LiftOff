...
Compilation, unit tests, and mocked-browser smoke coverage establish the command contracts, settings migration, tray composition, and app-poll gating, but Ally/WebView2 validation remains required. Confirm on the ROG Ally that Core Audio reads and changes the active render endpoint without flyout spam; raising volume unmutes; WMI brightness appears, changes the internal panel on AC and battery, and stays hidden on external-monitor-only systems; Smart/Full/Hidden layouts paint correctly at 1280x800 across representative Glass/Material/win9x surfaces; the shortcut/system-control/pinned/Spotify priority order remains legible, sufficiently opaque over bright game art, and stable as focus moves; long pinned rows keep the focused shortcut visible; pinned games open Details and pinned apps launch without A-button bleed; hidden hover and activity peeks time out correctly; Spotify transport/seek and playlist entry points behave for Premium, Free, disconnected, and no-track states; single-press MENU tray access in every bar mode, A-gated scrubber/system-control adjustment, adjacent-control navigation, D-pad repeat, B close, Controls LB/RB, and tray-to-Power/modal transitions never leak an activation underneath.

### Lo-fi scene video playback

Settings, locale, and `<video>` reload wiring cover `lofi_scene` (`cozy`, `dog`, `desk`, `cat`, `rainy_street`, `pixel_shop`) plus the Settings picture picker and the onboarding scene step when Lo-fi is selected. Ally X WebView2 validation remains required: switch to Lo-fi, open the scene picker and choose all six stills, confirm the background video swaps live, the separate looping mp3 keeps playing, effects-off pauses the clip, a non-`cozy` choice survives restart, and first-run/re-run onboarding shows the scene pictures only after Lo-fi is picked.
