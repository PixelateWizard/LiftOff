...
- Non-browser app launches use the same window watcher as games. Shortcut, shell, and elevated apps no longer fast-dismiss after a fixed delay; a 15-second miss emits `launch-unconfirmed` and the overlay stays until the user dismisses. Browser Cloud / http(s) launches keep bounded fast-success because they cannot expose a distinct child window.
