...
- The helper tray and Controls modal own their keyboard/gamepad loops while visible and gate the app poll through mirrored refs. Minimal mode reserves a single MENU press for the helper tray; Full and Hidden preserve tab-specific MENU taps and use an 850 ms MENU hold for the tray regardless of Spotify state. Inside the tray, Volume and Brightness require A to enter adjustment mode so Left/Right can otherwise move between them; A or B exits adjustment.
