...
- The helper tray and Controls modal own their keyboard/gamepad loops while visible and gate the app poll through mirrored refs. A single MENU press opens the helper tray in Full, Minimal, and Hidden modes; suppress that edge until release so it cannot also activate the tray or the underlying tab. Inside the tray, the Spotify scrubber, Volume, and Brightness require A to enter adjustment mode so Left/Right can otherwise move between neighboring controls; A or B commits and exits adjustment. Reserve controller-hint space even while unfocused so focus changes never reflow the tray.
