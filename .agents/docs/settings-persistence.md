...
- `nav_bumpers_pos` `bottom` is only offered when the Full bar is showing. Smart and Hidden resolve a stored `bottom` value to `hidden` without renaming the wire value for Full users. Fresh defaults use `header` bumpers and `tabbar_text_tabs: true` with medium label weight.
