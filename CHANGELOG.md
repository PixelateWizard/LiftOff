...
### Added
- **Spotify in-app playback target** - LiftOff now creates its own Spotify Web Playback SDK device ("LiftOff on Ally") and targets playlist/play actions at that local device when ready, so Spotify audio can play through the Ally/app instead of defaulting to another Spotify Connect device. New Spotify auth sessions request the required streaming scope.
...
### Fixed
- **Spotify SDK state sync** - the in-app Spotify player now polls the SDK's real playback state instead of advancing progress optimistically, removes the duplicate overlay progress bar, debounces playlist/play starts, starts playlists from offset 0, and surfaces SDK autoplay failures.
- **Spotify playback polish** - Spotify transport controls no longer report `parse failed` on empty successful responses, the in-app player feeds live now-playing/progress state to the mini-bar and overlay scrubber, and playlist grid gamepad navigation now moves vertically by the rendered row size instead of a hardcoded column count.
