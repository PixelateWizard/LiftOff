...
## ⚡ Active Task
- Implemented the GPU TDR Reduction PR (`pr-gpu-tdr-reduction.md`) to reduce WebView2 GPU/VRAM pressure that could cause `DXGI_ERROR_DEVICE_RESET` (TDR) on shared-GPU handhelds like the ROG Ally. Backend: added `hide_for_launch` (hides the WebView2 window ~400ms after a launch handoff to release D3D compositor resources) and `show_after_launch` (restores + focuses) Tauri commands, registered them, and added a default-on `#[serde(default = "default_true")] hide_on_launch` Settings field (mirrored in Rust struct, Default impl, TS `Settings`, and `constants.ts` defaults; no UI toggle yet per the PR). Added GPU Chromium flags `--disable-gpu-memory-buffer-video-frames --enable-zero-copy --num-raster-threads=2` to `tauri.conf.json` `app.windows[0].additionalBrowserArgs` (did not add `--disable-gpu`/`--disable-gpu-compositing`). Frontend: `_triggerLaunchImpl` calls `hide_for_launch` after `launchApp` when `hide_on_launch !== false`; `closeLaunchOverlay` calls `show_after_launch` before clearing launch state; `resumeFromBackground` safety-net calls `show_after_launch` when focus returns while `launchingAppRef` is set. Throttle background effects when unfocused: `AppBackground` now takes `windowFocused` and passes `effectsActive = effectsEnabled && windowFocused && !appPaused` to `CyberpunkBg`/`WebcoreBg`; ascii-rain skips its per-frame canvas draw entirely when `speed` is 0; App.jsx sets `data-focus` on the root div and a new `[data-focus="blurred"]` CSS rule pauses all theme background animations (the data-effects and data-focus selector lists were refactored to share one `bgAnimClasses` source so they can't drift). Validated with `npm.cmd run build` (only the existing Vite chunk-size warning), `cargo check` (only the existing `is_our_window_focused` unused warning), and `git diff --check` (only CRLF notices). Hands-on Ally testing of an actual game launch/return and the TDR behavior is still recommended.
